  ...
  for (let i in oList) {
    ...
    if ((idx = findOrder(ret, OrderID)) === -1) {
      ret.push({
        ...
        ProName: [],
        Stock: [],
        fullName: oList[i].Name,
        Date: null,
      });
      idx = ret.length - 1;
    }
    ret[idx]["Date"] = oList[i]["Date"];
    ret[idx]["ProName"].push(oList[i]["ProName"]);
    ret[idx]["Stock"].push(oList[i]["Stock"]);
  }
  console.log(ret);
  res.render("admin/order", {
    oActive,
    ordList: ret,
    empty: oList.length === 0,
    layout: "admin.hbs",
  });
});
